...
class IntervalProcessor(DataProcessor):
    ...
    def delete(self, domain: str, **parameters: Type[TypedDataFrame]) -> None:
        ...
        derrived_domain = existing_domain - SparsityMappingString(unit=self.unit, string=domain)
        new_domain = existing_domain - derrived_domain

        #loop over derrived domain and delete data
        for interval in derrived_domain.get_intervals():
            ...
            data = self.storage.retrieve_data(self.name, query=deletion_query)
            # self.storage.delete_rows(self.name, data.uuid)
